...
static int sja1105_adjust_port_speed(struct sja1105_port *port, int speed_mbps)
{
	...
	/* Ceck if current speed == new speed -> do nothing */
...
void sja1105_netdev_adjust_link(struct net_device *net_dev)
{
	...
	if (port->phy_dev->link) {
		...
		rc = sja1105_adjust_port_speed(port, port->phy_dev->speed);
                  ...
                  struct rtnl_link_stats64 *storage)
{
	...
	errcnt = status.n_n664err + status.n_vlanerr + status.n_unreleased +
	         ...
	         status.n_ctpolerr + status.n_polerr;

	port->stats.rx_packets = (status.n_rxfrmsh << 32) + status.n_rxfrm;
	port->stats.tx_packets = (status.n_txfrmsh << 32) + status.n_txfrm;
	port->stats.rx_bytes   = (status.n_rxbytesh << 32) + status.n_rxbyte;
	port->stats.tx_bytes   = (status.n_txbytesh << 32) + status.n_txbyte;
